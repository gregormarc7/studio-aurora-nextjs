...
export default function SectionApartma() {
  ...
  const prevImage = () => handleImageChange((selectedImageIndex === 0 ? images.length - 1 : selectedImageIndex - 1))
  const nextImage = () => handleImageChange((selectedImageIndex === images.length - 1 ? 0 : selectedImageIndex + 1))
  ...
  return (
    <section id="apartma" className="py-16 bg-gray-100 text-gray-800">
      <div className="container mx-auto px-4 text-center">
        ...
        <div className="relative group mx-auto w-full max-w-4xl rounded-xl overflow-hidden">
          <img
        ...
        <div className="grid grid-cols-5 gap-4 mt-6">
          {images.map((src, index) => (
            <img
              ...
              loading="lazy"
              className={`cursor-pointer h-24 object-cover w-full rounded-md border-2 transition-opacity duration-300 ${index === selectedImageIndex ? 'border-blue-500' : 'border-transparent'}`}
      ...
      <Dialog open={isOpen} onClose={closeModal} className="relative z-50">
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          <Dialog.Panel className="relative bg-white rounded-lg max-w-4xl w-full overflow-hidden shadow-lg">
            <button
              onClick={closeModal}
